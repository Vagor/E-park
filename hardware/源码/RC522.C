...
/************************************************
这是RC522刷卡模块的驱动函数，和单片机采用的是SPI通信，具体SPI接口可以百度一下。
这是模块本身自带的驱动函数，可以用在很多的单片机上面，不仅仅是51单片机，当然用其他单片机的时候
具体的一些时序可能要稍微改动。
所以这个文件的函数你可以不用明白，知道怎么用就行。很多都是可以通用的。
单片机要可以读到卡片，具体的步骤就是：寻卡，防碰撞，选卡，密码验证
************************************************/
...
{
	unsigned char SPICount;                                       // 计数器用来计时数据
